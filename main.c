	...
	while ( *++argv ) {
			...
			while ( argv [0][index] ) {
				switch ( argv [0][index] ) {
					...
					if ( delay != DEFAULT_DELAY ) {
						if ( ! ( warning & 2 ) ) {
							fprintf ( stderr,"Warning - speed setting has no effect when analysis option selected.\n");	
							warning |= 2;
						}
					}
					analysis = 1;
					...
					} else if ( delay == DEFAULT_DELAY ) delay = FAST_DELAY;
					else if ( ! ( warning & 1 ) ) {
						warning |= 1;
						fprintf ( stderr,"Warning - speed already set, discarding last speed setting.\n" );
					}
					break;
				  	case 's':
					if ( analysis ) {
					...
					break;
			  		default:
					warning += 4;
					fprintf ( stderr,"Unknown option letter (-%c) ignored.\n", argv [0][index] );
					break;
				}	
				++index;
			}
		}
	 	else fprintf ( stderr,	"Illegal command line parameter (%s) ignored.\n", *argv );
   	}
   	...
   	if ( help ) {
		...
		if ( status == 1 ) {
			...
			if ( technique & (0x100) ) fprintf ( stdout,"\tBlock cross hatch scan\n" );
			if ( technique & (0x200) ) fprintf ( stdout,"\tRow/column cross hatch scan\n" );
			if ( technique & (0x400) ) fprintf ( stdout,"\tMissing digit scan\n" );
			if ( technique & (0x2000) ) fprintf ( stdout,"\tRegion intersection analysis\n" );
			if ( technique & (0x800) ) fprintf ( stdout,"\tNaked %s analysis\n",printTuple ( GNL ( technique ) ) );
			if ( technique & (0x1000) ) fprintf ( stdout,"\tHidden %s analysis\n",printTuple ( GHL ( technique ) ) );
			if ( technique & (0x4000) ) fprintf ( stdout,"\tWhat if analysis\n" );
